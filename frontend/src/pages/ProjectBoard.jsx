      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        ...
        {task.trackedTime > 0 && (
          ...
          >
            ⏱{" "}
            {task.trackedTime >= 60
              ? `${Math.floor(task.trackedTime / 60)}h ${task.trackedTime % 60}m`
              : `${task.trackedTime}m`}
          </span>
        )}
      </div>
    ...
    >
      ...
      >
        <div style={{ minWidth: 0 }}>
          <Link
            to="/projects"
            style={{ fontSize: 13, color: C.sub, textDecoration: "none" }}
          >
            ← Projects
          </Link>
          <h1
          ...
          <p style={{ fontSize: 14, color: C.sub, margin: 0 }}>
            {tasks.length} task{tasks.length !== 1 ? "s" : ""} · drag to change
            status
          </p>
        </div>
        <Link
